...
function importSave(imported=undefined, forced=false) {
	...
	try {
		...
		if(tempPlr.versionType != modInfo.id && !forced && !confirm("这个存档来自另外一个 MOD! 是否导入？")) // Wrong save (use "Forced" to force it to accept.)
			return
		player = tempPlr;
...
function respecBuyables(layer) {
	...
	if (!confirm("你确定要重置科技树？你将会强制进行一次 \"" + (tmp[layer].name ? tmp[layer].name : layer) + "\" 重置！")) return
