...
function formatTime(s) {
	...
	else return formatWhole(Math.floor(s/3600))+" 时 "+formatWhole(Math.floor(s/60)%60)+" 分 "+format(s%60)+" 秒"
}
...
function load() {
	...
	else player = Object.assign(getStartPlayer(), JSON.parse(decodeURIComponent(atob(get))))
	fixSave()
...
function importSave(imported=undefined, forced=false) {
	if (imported===undefined) imported = prompt("在此处粘贴你的存档")
	try {
		tempPlr = Object.assign(getStartPlayer(), JSON.parse(decodeURIComponent(atob(imported))))
		if(tempPlr.versionType != modInfo.id && !forced && !confirm("你的存档似乎来自一个不同的 MOD! 你确定要导入？")) // Wrong save (use "Forced" to force it to accept.)
			return
		player = tempPlr;
...
function respecBuyables(layer) {
	...
	if (!confirm("你确定要重置树？你将会强制进行一次 \"" + (tmp[layer].name ? tmp[layer].name : layer) + "\" 重置！")) return
