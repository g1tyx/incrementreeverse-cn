...
// data = a function returning the content (actually HTML)
	...
	Vue.component('challenge', {
		...
		<div v-if="tmp[layer].challenges && tmp[layer].challenges[data]!== undefined && tmp[layer].challenges[data].unlocked && !(player.hideChallenges && hasChallenge(layer, [data]))" v-bind:class="{hChallenge: true, done: finishedChallenges(layer, data), canComplete: player[layer].activeChallenge == data&&!finishedChallenges(layer, data)&&canCompleteChallenge(layer, data)}">
			...
			<button v-bind:class="{ longUpg: true, can: true, [layer]: true }" v-bind:style="{'background-color': tmp[layer].color}" v-on:click="startChallenge(layer, data)">{{player[layer].activeChallenge==(data)?(canCompleteChallenge(layer, data)?"完成":"提前退出"):(finishedChallenges(layer, data)?"已完成":"开始")}}</button><br><br>
			<span v-html="tmp[layer].challenges[data].challengeDescription"></span><br>
			挑战目标：{{format(tmp[layer].challenges[data].goal)}} {{tmp[layer].challenges[data].currencyDisplayName ? tmp[layer].challenges[data].currencyDisplayName : "点数"}}<br>
			奖励：<span v-html="tmp[layer].challenges[data].rewardDescription"></span><br>
			<span v-if="tmp[layer].challenges[data].rewardEffect!==undefined">当前奖励： <span v-html="(tmp[layer].challenges[data].rewardDisplay) ? (tmp[layer].challenges[data].rewardDisplay) : format(tmp[layer].challenges[data].rewardEffect)"></span></span>
		</div>
		`
	})
	...
	Vue.component('upgrade', {
		...
		<button v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!== undefined && tmp[layer].upgrades[data].unlocked" v-on:click="buyUpg(layer, data)" v-bind:class="{ [layer]: true, upg: true, bought: hasUpgrade(layer, data), locked: (!(canAffordUpgrade(layer, data))&&!hasUpgrade(layer, data)), can: (canAffordUpgrade(layer, data)&&!hasUpgrade(layer, data))}"
			...
			<span v-if="tmp[layer].upgrades[data].effect"><br>当前效果： <span v-html="(tmp[layer].upgrades[data].effectDisplay) ? (tmp[layer].upgrades[data].effectDisplay) : format(tmp[layer].upgrades[data].effect)"></span></span>
	...
	Vue.component('main-display', {
		props: ['layer'],
		template: `
		<div><span v-if="player[layer].points.lt('1e1000')">你拥有 </span><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px' + tmp[layer].color}">{{formatWhole(player[layer].points)}}</h2> {{tmp[layer].resource}}<span v-if="tmp[layer].effectDescription">, {{tmp[layer].effectDescription}}</span><br><br></span>
		`
	})
	...
	Vue.component('main-resouce-display', {
		props: ['layer'],
		template: `
		<div><span v-if="player[layer].points.lt('1e1000')">你拥有 </span><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px' + tmp[layer].color}">{{formatWhole(player[layer].points)}}</h2> {{tmp[layer].resource}}<br></span>
		`
	})
	...
	Vue.component('resource-display', {
		...
		<div style="margin-top: -13px">
			<br>
			<span v-if="player[layer].best != undefined">{{tmp[layer].resource}}的最大数量是 {{formatWhole(player[layer].best)}}<br></span>
			<span v-if="player[layer].total != undefined">你总共制造了 {{formatWhole(player[layer].total)}} {{tmp[layer].resource}}<br></span>
		</div>
		`
		/*
		`
	...
	Vue.component('layer-node', {
		...
		<button v-if="nodeShown(layer)"
			...
			v-bind:tooltip="
				...
				: (tmp[layer].tooltipLocked ? tmp[layer].tooltipLocked : 'Reach ' + formatWhole(tmp[layer].requires) + ' ' + tmp[layer].baseResource + ' to unlock (你拥有 ' + formatWhole(tmp[layer].baseAmount) + ' ' + tmp[layer].baseResource + ')')
			"
			v-bind:class="{
				treeNode: size != 'small',
